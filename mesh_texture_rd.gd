...
var texture_rd: RID = RenderingServer.texture_2d_placeholder_create()
...
@export var glsl_file: RDShaderFile = preload("res://glsl/base_texture.glsl"):
    get: return glsl_file
    set(value):
        if (glsl_file != null): glsl_file.changed.disconnect(_queue_update_pipeline)
        glsl_file = value
        _queue_update_pipeline()
        if (glsl_file != null): glsl_file.changed.connect(_queue_update_pipeline)
...
func _init() -> void:
    ...
    glsl_file.changed.connect(_queue_update_pipeline)
...
func _reset_vertex() -> void:
    ...
    vertex_array_rid = rd.vertex_array_create(pointsBytes.size() / 12, vertex_format, vertexBuffers)
...
func _draw_list() -> void:
    if not (pipeline_rid.is_valid() and framebuffer_rid.is_valid() and vertex_array_rid.is_valid() and uniform_set_rid.is_valid()):
        return
